...
function generaOpzioni(corretta: string) {
  const altre = TUTTE_FORME.filter(a => a !== corretta)
  const distrattori = mischia(altre).slice(0, 3)
  return mischia([corretta, ...distrattori])
}

function riempiFrase(frase: string, articolata: string) {
  return frase.replace('___', articolata)
}

function PreposizioniGame({ onBack, onStarEarned }: { onBack: () => void; onStarEarned: (key: string) => void }) {
  ...
  const [risposte, setRisposte] = useState<{ domanda: string; corretta: boolean; rispostaCorretta: string; rispostaData: string; valoreSbagliato: string | null }[]>([])
